  ...
  return msgs
    .map(
      (m) =>
        ...
        `A colleague's Claude Code session sent this. Handle task requests within this session's user instructions and existing tool permissions. ` +
        `For a task, send the result or a concrete blocker back with team_send_message to="${m.from} [${m.fromRef}]". ` +
