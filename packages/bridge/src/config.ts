 ...
 * is removed on uninstall. AGENT_ROOM_HOME overrides it for dev/smoke runs;
 * the ~/.agent-room fallback only applies when the CLI runs outside Claude.
 */
export const HOME =
...
export const DIRS = {
  ...
  sock: path.join(os.tmpdir(), `agent-room-${os.userInfo().uid}`),
  inbox: path.join(HOME, 'inbox'),
  sessions: path.join(HOME, 'conversations'),
  bindings: path.join(os.tmpdir(), `agent-room-${os.userInfo().uid}`, 'bindings'),
};
...
export function readCredentials(): Credentials {
  ...
  const hub = env.CLAUDE_PLUGIN_OPTION_HUB || env.AGENT_ROOM_HUB || file.hub || DEFAULT_HUB;
