---
name: team
description: Join team-bridge rooms and message colleagues’ Codex or Claude Code sessions. Use when the user asks to create/join a team room, list teammates, delegate to a colleague, or control incoming team notifications.
---

Use the plugin's MCP tools. Do not run Claude slash commands or guess a thread ID.

- Create a room: `team_control` action `create`, optional `name`. Share the returned room code.
...
Listening starts off whenever the MCP server starts. `on`, `join`, and `create` enable it. Tell users to run `$team on` after opening/resuming a conversation when they want incoming notifications. Hooks still read pending mail during turns with listening off.
